...
- [x] Utilise render function props from QuestionForm for each field type, applying the relevant type to the prop parameters (negating the switch statement)
...
|          |                                                            |                                                                                                                                              |                                                                                                                                                         |       |
| OPTION 3 | Schema-driven form with Formik and Async Validator library | - Formik dependency already in use in frontend UI                                                                                            |                                                                                                                                                         |       |
|          |                                                            | - Most popular open source form library                                                                                                      |                                                                                                                                                         |       |
|          |                                                            | - Least risk as the form implementation and validation are decoupled, allowing either to be modified or replaced independently               | - Complexities around waiting for specific combinations of answers to trigger external behaviour (i.e. submit early if specific question path answered) |       |
|          |                                                            | - Bespoke to current requirements                                                                                                            | - Only as flexible as the currently developed behaviour (field types: RadioGroup, TextInput, LinkButton)                                                |       |
|          |                                                            | - Built upon widely used open source form library                                                                                            | - Transition logic for conditional questions must be defined for EACH answer for EACH question (gaps WILL break the flow)                               |       |
|          |                                                            | - Easily extended with new field types                                                                                                       | - Future developer must take care if modifying/extending the schema in future, so the form doesn't break                                                |       |
|          |                                                            | - Provides better support for async-validator library                                                                                        | - In small cases may impose restrictions on how child questions are rendered (positioning relative to parent questions)                                 |       |
|          |                                                            | - Easy predictability of form flow with state-machine-like transition to next questions                                                      | - Time required to introduce an "on form change" callback to notify any subscriber (i.e. HOC or state) of a field change                                |       |
